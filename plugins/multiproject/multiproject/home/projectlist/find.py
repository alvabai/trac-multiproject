...
from multiproject.home.projectlist.categorybrowser import CategoryBrowser
...
class FindProjectsModule(Component):
    ...
    def process_request(self, req):
        ...
        if not user.isProjectBrowsingAllowed():
    ...
    def _filtered_resultset_partial(self, req, rss = False):
        ...
        if project_count:
            if tab == 'recent':
                ...
                showing_clause = _("Showing %(start)s - %(end)s most active projects out of %(total)s",
                    start = (sub_page - 1) * numresults + 1,
                    end = min(sub_page * numresults, project_count),
                    total = project_count)
        else:
        ...
        if not rss:
            ...
            categories = CategoryBrowser().find_categories(category_id_list)
            cat_string = ""
            sec_string = ""
            if len(categories) == 1:
    ...
    def _show_regular_page(self, req):
        ...
        add_script(req, 'multiproject/js/explore_projects.js')

        return "find.html", {'top_categories':req_data['top_categories'],
