...
function select_category(cat_id, top_cat) {
    ...
    this_list.parentsUntil('.rootcatlist').prev('.category_item').add(this_list)
        .each(function () {
    ...
    cat_list_item.find('.category_checkbox input[type=checkbox]').attr('checked','checked');

    // Set style to "selected" for both top and non-top category list
    cat_list_item.addClass('selected');
    ...
    cat_list_item.parentsUntil('.rootcatlist').prev('.category_item').filter('.selected')
    ...
    cat_list_item.parentsUntil('.rootcatlist').prev('.category_item').addClass('sub_cat_selected');
    get_children_list(cat_list_item).find('.category_item.selected')
        .removeClass('selected').each(function() {
    ...
    get_children_list(cat_list_item).find('.category_item.sub_cat_selected').add(cat_list_item)
        .removeClass('sub_cat_selected');
    //console.log('adding children_in_search to', cat_list_item);
    get_children_list(cat_list_item).addClass('children_in_search');
...
function put_category_into_search_box(cat_id) {
    ...
    var cat_name = $('#cat_'+cat_id+'_top').add('#cat_'+cat_id).first().text();
...
function unselect_category(cat_id, top_cat) {
    ...
    // For the category in both non-top and top category
    cat_list_item.find('.category_checkbox input[type=checkbox]').attr('checked','');
    ...
    while (sub_category_list.length > 0) {
        ...
        sub_category_list.prev('.category_item.sub_cat_selected').removeClass('sub_cat_selected');
...
function get_children_list(list_item) {
    return list_item.next('.subcatlist');
}
...
function init_view() {
    ...
    $(".catlist span.category_checkbox > input[type=checkbox]").click(
        function() {
    ...
    $(".context_item_top").click(function () {
        var list_item = $(this);
        if (list_item.hasClass('opened')) {
            close_list_item(list_item);
        }
    ...
    $("#all_categories_toggle, #toggle_img").click(
        function() {
    ...
    $('#tabbedNavigation li').click(function() {
...
function read_fragment() {
    ...
    for(var name in search_params) {
        if(name == 'c') {
            ...
            for(var index in search_params[name]) {
                var cat_id = search_params[name][index];
                var is_top = false;
                if (cat_id.indexOf('_top') != -1) {
                    is_top = true;
                    cat_id = cat_id.replace('_top','');
